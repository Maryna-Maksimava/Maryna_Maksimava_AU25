...
with_lags as (
    -- 2) add lags/leads and weekly cumulative sum
    select
        d.*,
...
select
    ...
    round(
        case
            -- monday: sat + sun (lag2, lag1) + mon + tue (lead1)
            ...
            -- other days: centered 3-day avg = prev + current + next
            else
                (
                    coalesce(amt_lag1, 0) +
                    amount_sold +
...
from with_lags
where calendar_week_number between 49 and 51   -- 3) report weeks 49–51 only
order by time_id;
------------------------------------
